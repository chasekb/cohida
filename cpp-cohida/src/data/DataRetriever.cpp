...
// Include Config.h if not already (it is included in DataRetriever.h or we need
// to add it) Checking includes... DataRetriever.cpp includes
// "data/DataRetriever.h" We need to ensure Config.h is available.

DataRetriever::DataRetriever() : is_retrieving_(false) {
  auto &cfg = config::Config::get_instance();
  coinbase_client_ = std::make_unique<api::CoinbaseClient>(
      cfg.get_string("COINBASE_API_KEY", ""),
      cfg.get_string("COINBASE_API_SECRET", ""),
      cfg.get_string("COINBASE_API_PASSPHRASE", ""));
...
DataRetrievalResult DataRetriever::retrieve_historical_data(
    const std::string &symbol, system_clock::time_point start_date,
    system_clock::time_point end_date, int granularity) {
...
std::vector<models::CryptoPriceData>
DataRetriever::_fetch_data_from_api(const DataRetrievalRequest &request) {
  try {
    auto candles = coinbase_client_->get_historical_candles(
        request.symbol, request.start_date, request.end_date,
        request.granularity);

    LOG_DEBUG("API returned " + std::to_string(candles.size()) +
              " candles for symbol: " + request.symbol);
    return candles;
...
std::vector<models::CryptoPriceData>
DataRetriever::_transform_api_data(const std::vector<json> &raw_data,
  ...
  for (const auto &candle : raw_data) {
    try {
      ...
      auto timestamp =
          system_clock::time_point(seconds(static_cast<long long>(candle[0])));

      models::CryptoPriceData data_point(
          symbol, timestamp, models::Decimal(candle[3].get<std::string>()),
  ...
  LOG_DEBUG("Transformed " + std::to_string(data_points.size()) +
            " data points from API response");
  return data_points;
}
      ...
      if (test_result.success && !test_result.data_points.empty()) {
        auto min_timestamp =
            std::min_element(test_result.data_points.begin(),
                             ...
                             [](const models::CryptoPriceData &a,
                                const models::CryptoPriceData &b) {
...
DataRetrievalResult
DataRetriever::retrieve_all_historical_data(const std::string &symbol,
                                            int granularity,
  ...
  try {
    ...
    while (chunk_start < end_date) {
      ...
      LOG_INFO("Processing chunk " + std::to_string(chunk_count + 1) + ": " +
               format_time_point(chunk_start) + " to " +
               format_time_point(chunk_end));
      ...
      try {
        ...
        if (chunk_result.success && !chunk_result.data_points.empty()) {
          ...
          LOG_DEBUG("Chunk " + std::to_string(chunk_count + 1) + " retrieved " +
                    std::to_string(chunk_result.data_points.size()) +
                    " data points");
        } else {
      ...
      if (max_records &&
          static_cast<int>(all_data_points.size()) >= *max_records) {
        LOG_INFO("Reached maximum record limit: " +
                 std::to_string(*max_records));
        all_data_points.erase(all_data_points.begin() + *max_records,
                              all_data_points.end());
        break;
      }
    }
...
void DataRetriever::_log_retrieval_progress(int current, int total) {
  if (total > 0) {
    ...
    LOG_INFO("Data retrieval progress: " +
