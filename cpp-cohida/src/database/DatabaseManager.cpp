...
string_to_time_point(const std::string &s) {
  std::tm tm = {};
...
DatabaseManager::~DatabaseManager() { close_connections(); }
...
void DatabaseManager::_initialize_connection_pool() {
  try {
    ...
    LOG_ERROR("Failed to initialize database connection pool: " +
              std::string(e.what()));
    throw DbException("Failed to initialize database connection pool: " +
                      std::string(e.what()));
  }
}
  ...
  if (connection_pool_.size() < static_cast<size_t>(MAX_POOL_SIZE)) {
    try {
      ...
      LOG_ERROR("Failed to create new database connection: " +
                std::string(e.what()));
      throw DbException("Failed to create new database connection: " +
                        std::string(e.what()));
    }
  }
...
void DatabaseManager::_return_connection(
...
void DatabaseManager::_ensure_schema_exists() {
  ...
  try {
    ...
    // Create indexes
    std::string symbol_timestamp_index =
    ...
    throw DbException("Failed to ensure schema exists: " +
                      std::string(e.what()));
  }
}
...
std::vector<models::CryptoPriceData> DatabaseManager::read_data(
  ...
  try {
    ...
    auto result = txn.exec(pqxx::zview(select_sql),
                           pqxx::params{symbol, format_time_point(start_date),
                                        format_time_point(end_date)});
    ...
    throw DbException("Failed to read data from database: " +
                      std::string(e.what()));
  }
...
int DatabaseManager::get_data_count(const std::string &symbol) {
  try {
    ...
    LOG_ERROR("Failed to get data count for " + symbol + ": " +
              std::string(e.what()));
    return 0;
  }
}
...
DatabaseManager::get_latest_timestamp(const std::string &symbol) {
  try {
    auto conn = _get_connection();
    pqxx::work txn(*conn);
    ...
    LOG_ERROR("Failed to get latest timestamp for " + symbol + ": " +
              std::string(e.what()));
    return std::nullopt;
  }
}
