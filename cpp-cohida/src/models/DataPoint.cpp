...
bool CryptoPriceData::isValid() const {
    ...
    if (open_price <= zero || high_price <= zero ||
        low_price <= zero || close_price <= zero) {
...
SymbolInfo::SymbolInfo(std::string symbol,
...
bool DataRetrievalRequest::isValid() const {
    ...
    auto it = std::find(valid_granularities.begin(), valid_granularities.end(), granularity);
    ...
    if (!skip_validation) {
        ...
        if (duration > max_duration) {
            LOG_WARN("Date range too large for granularity " + std::to_string(granularity) +
...
DataRetrievalResult::DataRetrievalResult(std::string symbol,
                                       bool success,
...
int DataRetrievalResult::data_count() const {
    return data_points.size();
}

bool DataRetrievalResult::is_empty() const {
...
bool SymbolValidator::is_valid_symbol(const std::string& symbol) {
    ...
    if (base.length() < 2 || base.length() > 10 || quote.length() != 3) {
        return false;
    }
