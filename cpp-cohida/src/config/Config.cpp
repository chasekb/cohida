...
std::optional<std::string> Config::get_optional_string(const std::string& key) const {
    auto it = config_.find(key);
    if (it != config_.end()) {
        return it->second;
    }
...
std::optional<double> Config::get_optional_double(const std::string& key) const {
    auto it = config_.find(key);
    if (it != config_.end()) {
        try {
            return std::stod(it->second);
