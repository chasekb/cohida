...
TEST_F(CoinbaseClientTest, Initialization) {
...
TEST_F(CoinbaseClientTest, GetCurrentPrice) {
    ...
    if (price) {
        EXPECT_GT(price->convert_to<double>(), 0.0) << "Price should be positive";
    }
}
...
TEST_F(CoinbaseClientTest, GetHistoricalCandles) {
    ...
    auto candles = client->get_historical_candles("BTC-USD", one_day_ago, now, 3600);
    
    EXPECT_GT(candles.size(), 0) << "Failed to retrieve historical candles";
    
    for (const auto& candle : candles) {
        EXPECT_EQ(candle.symbol, "BTC-USD");
        EXPECT_GT(candle.close_price.convert_to<double>(), 0.0);
        EXPECT_GT(candle.volume.convert_to<double>(), 0.0);
