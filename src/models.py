...
class DatabaseSchema:
    ...
    CREATE_INDEXES_SQL = [
        "CREATE INDEX IF NOT EXISTS idx_crypto_data_symbol ON crypto_historical_data(symbol);",
        "CREATE INDEX IF NOT EXISTS idx_crypto_data_timestamp ON crypto_historical_data(timestamp);",
        "CREATE INDEX IF NOT EXISTS idx_crypto_data_symbol_timestamp ON crypto_historical_data(symbol, timestamp);"
    ]
    
    INSERT_DATA_SQL = """
    INSERT INTO crypto_historical_data (symbol, timestamp, open_price, high_price, low_price, close_price, volume)
    VALUES (%(symbol)s, %(timestamp)s, %(open_price)s, %(high_price)s, %(low_price)s, %(close_price)s, %(volume)s)
    ON CONFLICT (symbol, timestamp) DO UPDATE SET
        open_price = EXCLUDED.open_price,
        high_price = EXCLUDED.high_price,
